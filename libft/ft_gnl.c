...
static void	mem_to_line(char **line, char **mem, char *str, int *r)
{
	...
	{
		*r = 1;
		if (!ft_strstr(*mem, str) && !ft_strchr(*mem, '\n'))
		{
			*line = *mem;
			*mem = NULL;
		}
		...
		{
			e = (ft_strstr(*mem, str) > ft_strchr(*mem, '\n')) ? ft_strchr(*mem, '\n') - *mem: ft_strstr(*mem, str) - *mem;
			*line = ft_strnew(e + 1);
...
int			get_next(const int fd, char **line, char *str)
{
	...
	mem[fd].m = (mem[fd].fd != fd + 1) ? NULL : mem[fd].m;
	mem[fd].fd = fd + 1;
	r = 0;
	...
	!ft_strchr(mem[fd].m, '\n')))
		while ((r = read(fd, buf, BUFF_SIZE)))
		{
			...
			if (!(mem[fd].m = cw_stradd_chop(mem[fd].m, buf, BUFF_SIZE, 1)))
				return (-1);
			if (ft_strstr(mem[fd].m, str) || ft_strchr(mem[fd].m, '\n'))
				break ;
		}
